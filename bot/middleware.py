...
class UserInjectMiddleware(BaseMiddleware):
    """Injects user data into handler kwargs via backend API (no direct DB)."""
    ...
    async def __call__(
        ...
        try:
            result = await self._client.track_activity(
